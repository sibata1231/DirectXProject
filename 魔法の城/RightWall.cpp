...
HRESULT RightWall::Create(const char* passName, PrimitiveTypes primitiveType, ObjMaterial* material) {
    HRESULT hr      = S_OK;          
    ...
    // 頂点座標の設定
    pVtx[0].vtx = XMFLOAT3(0.0f, -m_transform->m_scale.x * 0.5f, -m_transform->m_scale.y * 0.5f);
    pVtx[1].vtx = XMFLOAT3(0.0f, -m_transform->m_scale.x * 0.5f,  m_transform->m_scale.y * 0.5f);
    pVtx[2].vtx = XMFLOAT3(0.0f,  m_transform->m_scale.x * 0.5f, -m_transform->m_scale.y * 0.5f);
    pVtx[3].vtx = XMFLOAT3(0.0f,  m_transform->m_scale.x * 0.5f,  m_transform->m_scale.y * 0.5f);

    // 法線の設定
    pVtx[0].nor = XMFLOAT3(1.0f, 0.0f, 0.0f);
    pVtx[1].nor = XMFLOAT3(1.0f, 0.0f, 0.0f);
    pVtx[2].nor = XMFLOAT3(1.0f, 0.0f, 0.0f);
    pVtx[3].nor = XMFLOAT3(1.0f, 0.0f, 0.0f);
...
void RightWall::Update() {
    Mesh::Update();
    XMMATRIX mtxWorld, mtxScale, mtxRotation, mtxTranslate;
	mtxWorld        = XMMatrixIdentity();                                                                                            // ワールドマトリックスの初期化
    mtxScale        = XMMatrixScaling(m_transform->m_scale.x, m_transform->m_scale.y, m_transform->m_scale.z);                       // 拡大縮小を反映
	mtxRotation     = XMMatrixRotationRollPitchYaw(m_transform->m_rotation.x, m_transform->m_rotation.y, m_transform->m_rotation.z); // 回転を反映
    mtxTranslate    = XMMatrixTranslation(m_transform->m_position.x, m_transform->m_position.y, m_transform->m_position.z);          // 移動を反映
	mtxWorld        *= mtxScale * mtxRotation * mtxTranslate;                                                                        // 全反映
	XMStoreFloat4x4(&m_transform->m_world, mtxWorld);                                                                             // ワールドマトリックスの設定
    m_animationTime += 0.01f;
    m_pom.m_vFrame   = m_animationTime;
}

void RightWall::Debug() {
    ...
    if (SUCCEEDED(context->Map(m_instanceBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &pData))) {
        ...
        for (int j = 0; j < MAX_INSTANCE_HEIGHT; ++j) {
            for (int i = 0; i < MAX_INSTANCE_WIDTH; ++i) {
                ...
                m._42 += (i - 1.65f) * 100.0f;
                m._43 += (j - 12.5f) * 100.0f;
...
// 頂点の作成
HRESULT RightWall::MakeMeshVertex(POM_VERTEX vertexWk[], int indexWk[]) {
...
DirectX::XMFLOAT3 RightWall::CalcTangent(POM_VERTEX p0, POM_VERTEX p1, POM_VERTEX p2) {
    // 5次元→3次元頂点に
