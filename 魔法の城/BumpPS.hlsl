...
float4 main(PS_IN PIN) : SV_TARGET {
    ...
    float2 texel     = PIN.uv + 0.5f * heightMap * PIN.eye.xy;           // 視線ベクトルによってずらす
    float3 normal    = 2.0f * tex[1].Sample(samp, uv).xyz - 1.0f;        // 法線取得
    float3 phong     = normalize(PIN.light + PIN.eye);                   // Phong
    float  specular  = pow(max(0.0f, dot(normal, phong)), 1.0f);         // Specular
