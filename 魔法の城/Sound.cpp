...
#include "SoundData.hpp"
	LPCWSTR g_pszDirSound = L"Resources/sound/";
...
void CSound::Init(void) {
	...
	if (m_pXAudio2) {
		m_pBgm = new CSoundStream[(int)BgmDatas::MAX];
		m_pSe = new CSoundEffect[(int)SeDatas::MAX];
		WCHAR szPath[_MAX_PATH];
		for (int i = 0; i < (int)BgmDatas::MAX; ++i) {
			lstrcpyW(szPath, g_pszDirSound);
			lstrcatW(szPath, g_pszDirBGM);
			lstrcatW(szPath, fileBGM[i]);
			if (FAILED(m_pBgm[i].Open(m_pXAudio2, szPath))) {
				::MessageBoxW(nullptr, szPath, L"BGM:error", MB_OK);
			}
		}
		for (int i = 0; i < (int)SeDatas::MAX; ++i) {
			lstrcpyW(szPath, g_pszDirSound);
			lstrcatW(szPath, g_pszDirSE);
			lstrcatW(szPath, fileSE[i]);
...
// BGM再生
void CSound::Play(BgmDatas bgm)
{
	if (!m_pBgm || (int)bgm < 0 || (int)bgm >= (int)BgmDatas::MAX) return;
	m_pBgm[(int)bgm].Play();
}

// SE再生
void CSound::Play(SeDatas se)
{
	if (!m_pSe || (int)se < 0 || (int)se >= (int)SeDatas::MAX) return;
	m_pSe[(int)se].Play();
}

// BGM再生中？
bool CSound::IsPlaying(BgmDatas bgm)
{
	if (!m_pBgm || (int)bgm < 0 || (int)bgm >= (int)BgmDatas::MAX) return false;
	return m_pBgm[(int)bgm].IsPlaying();
}

// BGM再生停止
void CSound::Stop(BgmDatas bgm)
{
	if (!m_pBgm || (int)bgm < 0 || (int)bgm >= (int)BgmDatas::MAX) return;
	m_pBgm[(int)bgm].Stop();
}
...
// BGMボリューム設定
void CSound::SetVolume(BgmDatas bgm, float fVol) {
	if (!m_pBgm || (int)bgm < 0 || (int)bgm >= (int)BgmDatas::MAX) {
		return;
	}
	m_pBgm[(int)bgm].SetVolume(fVol);
}

// BGMボリューム取得
float CSound::GetVolume(BgmDatas bgm) {
	if (!m_pBgm || (int)bgm < 0 || (int)bgm >= (int)BgmDatas::MAX) {
		return 0.0f;
	}
	return m_pBgm[(int)bgm].GetVolume();
}

// SEボリューム設定
void CSound::SetVolume(SeDatas se, float fVol) {
	if (!m_pSe || (int)se < 0 || (int)se >= (int)SeDatas::MAX) {
		return;
	}
	m_pSe[(int)se].SetVolume(fVol);
}

// SEボリューム取得
float CSound::GetVolume(SeDatas se) {
	if (!m_pSe || (int)se < 0 || (int)se >= (int)SeDatas::MAX) {
		return 0.0f;
	}
	return m_pSe[(int)se].GetVolume();
}
